...
def setup_driver(headless=False):
    """Setup Chrome WebDriver with options to avoid session conflicts"""
    options = Options()
    ...
    # Options to avoid session conflicts
    ...
    try:
        ...
        # Try without user-data-dir as a fallback
        ...
        options_fallback.add_argument("--incognito")

        try:
            driver = webdriver.Chrome(options=options_fallback)
            driver.implicitly_wait(10)
            print("✓ Chrome WebDriver started with fallback options")
            return driver
