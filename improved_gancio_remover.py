...
class GancioAPI:
    ...
    def login(self):
        ...
        found_indicators = [indicator for indicator in admin_indicators if indicator in admin_content]
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if isinstance(data, list) and len(data) > 0:
                                ...
                                print(f"✅ Found events in data structure via {endpoint}")
    ...
    def delete_event(self, event_id):
        ...
        csrf_match = re.search(r'name=["\']_token["\'] value=["\']([^"\']+)["\']', admin_page.text)
        if csrf_match:
        ...
        delete_attempts = [
            ...
            {'method': 'POST', 'url': f'/admin/events/{event_id}', 'data': {'_method': 'DELETE'}},
            
            # Form-based deletion
            {'method': 'POST', 'url': f'/admin/events/delete', 'data': {'id': event_id}},
        ]
...
def main():
    ...
    DELAY_BETWEEN_DELETES = 3  # Seconds to wait between deletions (increased for safety)
    ...
    try:
        ...
        for i, event in enumerate(kept_originals[:5]):
            ...
            if duplicates_count > 0:
                print(f"   {i+1}. ID: {event['id']}, Slug: {event['slug']} (keeping, deleting {duplicates_count} duplicates)")
        ...
        if DRY_RUN:
            ...
            if response == 'YES':
                print("⚠️  Switching to LIVE MODE - Events will be permanently deleted!")
                time.sleep(2)
                DRY_RUN = False
            else:
        ...
        if not DRY_RUN:
            ...
            print(f"   Processing in batches of {BATCH_SIZE} with {DELAY_BETWEEN_DELETES}s delays...")
            ...
            if deleted_count > 0:
                ...
                print(f"   Your event calendar should now show {len(kept_originals) + failed_count} events instead of {len(events)}.")
