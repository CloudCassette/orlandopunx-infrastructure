        ...
        for events_list in self.existing_events.values():
            for existing in events_list:
                ...
                if isinstance(ex_start, (int, float)):
                    existing_date = datetime.fromtimestamp(ex_start).strftime(
                        "%Y-%m-%d"
                    )
    ...
    def analyze_duplicates_in_gancio(self) -> Dict:
        ...
        for composite_key, events in self.existing_events.items():
            if len(events) > 1:
                ...
                print(
                    f"🔍 Found {len(events)} duplicates for: {events[0].get('title', 'Unknown')[:50]}..."
                )
        ...
        if dry_run:
            ...
            for composite_key, dup_info in duplicates.items():
                ...
                for remove_event in remove_events:
                    print(
                        f"   ❌ Would remove: [{remove_event['id']}] {remove_event['title'][:60]}..."
                    )
            return True
            ...
            for remove_event in remove_events:
                ...
                try:
                    ...
                    if response.status_code in [200, 204]:
                        ...
                        print(
                            f"   ❌ Failed to remove [{event_id}]: {response.status_code}"
                        )
...
def main():
    ...
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze existing duplicates"
    )
    ...
    parser.add_argument(
        "--force", action="store_true", help="Actually perform deletions"
    )
