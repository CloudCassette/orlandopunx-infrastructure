...
def validate_secrets():
    ...
    try:
        ...
        if not parsed_url.scheme or not parsed_url.netloc:
            ...
            print(f"   ✅ GANCIO_BASE_URL: Valid URL format ({parsed_url.scheme}://{parsed_url.netloc})")
    ...
    if len(password) >= 8:
        ...
        print(f"   ⚠️ GANCIO_PASSWORD: Short password ({len(password)} chars) - consider using a longer password")
    ...
    try:
        ...
        if 'admin' in auth_response.url.lower():
            ...
            if api_auth_response.status_code == 200:
                ...
                print(f"   ⚠️ Authenticated API access: {api_auth_response.status_code}")
