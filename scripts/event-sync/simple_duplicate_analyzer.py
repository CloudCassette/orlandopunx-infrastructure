...
def analyze_duplicates():
    ...
    exact_duplicates = {
        key: events_list
        for key, events_list in grouped_events.items()
    ...
    if not exact_duplicates:
        ...
        for composite_key, duplicate_events in exact_duplicates.items():
            ...
            for i, event in enumerate(duplicate_events):
                ...
                print(
                    f"   [{event.get('id', 'N/A')}] {status}: {event.get('title', 'No title')}"
                )
    ...
    for i, event1 in enumerate(events):
        ...
        for j, event2 in enumerate(events[i + 1 :], i + 1):
            ...
            if event1.get("start_datetime") == event2.get(
                "start_datetime"
            ) and event1.get("place", {}).get("name") == event2.get("place", {}).get(
                "name"
            ):

                # Check title similarity
                if titles_are_similar(
                    event1.get("title", ""), event2.get("title", ""), threshold=0.75
                ):
