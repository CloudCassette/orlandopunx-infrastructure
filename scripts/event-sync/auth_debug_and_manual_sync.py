...
class AuthDebugger:
    ...
    def debug_gancio_auth_methods(self):
        ...
        for endpoint in endpoints_to_check:
            try:
                ...
                print(f"   {endpoint}: HTTP {response.status_code} ({len(response.text)} chars)")
        ...
        try:
            ...
            if auth_response.status_code in [302, 301]:
                final_response = self.session.get(auth_response.headers.get('Location', '/'))
        ...
        for endpoint, data in api_endpoints:
            try:
                # Try JSON
                json_response = self.session.post(f"{self.gancio_base_url}{endpoint}", json=data)
                ...
                # Try form data
                form_response = self.session.post(f"{self.gancio_base_url}{endpoint}", data=data)
    ...
    def manual_event_creation_guide(self):
        ...
        print("Since automatic authentication failed, here's how to manually add events:")
