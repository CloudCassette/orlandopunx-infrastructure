...
class WillsPubGancioSync:
    ...
    def scrape_event_details(self, event):
        ...
        try:
            ...
            details = {"title": event["title"], "source_url": event["url"]}
    ...
    def run_sync(self, limit=5):
        ...
        for i, event in enumerate(events, 1):
            ...
            print(
                f"   📅 {event.get('start_date', 'No date')} at {event.get('start_time', 'No time')}"
            )
        ...
        print(f"\n" + "=" * 50)
        proceed = (
            input(f"Submit these {len(events)} events to your approval queue? (y/N): ")
            .strip()
            .lower()
        )
