...
def final_diagnosis():
    ...
    login_response = session.post("https://orlandopunx.com/login", data=login_data, allow_redirects=True)
    ...
    if events_before.status_code == 200:
        ...
        conduit_before = sum(1 for e in events_before.json() 
    ...
    if events_after.status_code == 200:
        ...
        conduit_after = sum(1 for e in events_after.json() 
                          if e.get('place', {}).get('name') == 'Conduit')
