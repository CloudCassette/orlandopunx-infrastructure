...
class AdvancedGancioDebugger:
    ...
    def print_environment_details(self):
        ...
        try:
            ...
            relevant_packages = [pkg for pkg in installed_packages 
                               if any(name in pkg.lower() for name in 
                                    ['requests', 'urllib', 'http', 'ssl'])]
    ...
    def test_authentication_flow(self):
        ...
        if login_success:
            ...
            try:
                auth_test_response = self.session.get(f"{self.base_url}/api/events",
                                                    timeout=self.timeout)
    ...
    def test_comprehensive_monitoring_compatibility(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ❌ Monitoring system will fail with status {response.status_code}")
    ...
    def run_full_diagnostic(self):
        ...
        api_connectivity = any(result.get('success', False) 
                             for result in connectivity_ok.values())
