...
class WorkingGancioCleanup:
    ...
    def find_admin_interface(self):
        ...
        for path in admin_paths:
            try:
                ...
                if any(
                    word in page_source
                    for word in ["event", "manage", "moderation", "approve"]
                ):
    ...
    def scan_for_events(self):
        ...
        try:
            ...
            cards = self.driver.find_elements(
                By.CSS_SELECTOR, ".card, .item, .event-item"
            )
            if cards:
            ...
            list_items = self.driver.find_elements(
                By.CSS_SELECTOR, "li:not([class*='nav']):not([class*='menu'])"
            )
            if list_items:
    ...
    def _is_nav_element(self, element):
        ...
        try:
            ...
            parent_classes = (
                element.find_element(By.XPATH, "..").get_attribute("class") or ""
            )
            ...
            return any(
                keyword in (classes + parent_classes).lower()
    ...
    def _extract_event_info(self, element, text):
        ...
        try:
            ...
            for line in lines[1:]:
                if any(
                    word in line.lower() for word in ["at ", "@", "venue", "location"]
                ):
                    venue = line
                    break
            ...
            for line in lines:
                if re.search(
                    r"\d{1,2}[/-]\d{1,2}|\d{4}-\d{2}-\d{2}|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec",
                    line,
                    re.I,
                ):
                    date = line
        ...
        for signature, events in duplicate_groups.items():
            ...
            for event in delete_events:
                try:
                        ...
                        try:
                            ...
                            if delete_btn:
                                self.driver.execute_script(
                                    "arguments[0].scrollIntoView();", delete_btn
                                )
                                time.sleep(0.5)
                                delete_btn.click()
                    ...
                    if not delete_found:
                        print(
                            f"   ⚠️ No delete button found for: {event['title'][:40]}..."
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Working Gancio browser duplicate cleanup"
    )
    parser.add_argument(
    ...
    cleaner = WorkingGancioCleanup(base_url=args.url, headless=not args.no_headless)
