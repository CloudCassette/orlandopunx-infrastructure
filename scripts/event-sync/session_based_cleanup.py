            ...
            try:
                ...
                for page in range(1, 21):  # Try up to 20 pages
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"    Page {page}: {len(events)} events ({new_count} new)")
    ...
    def cleanup_duplicates(self, groups: Dict, dry_run: bool = True) -> bool:
        ...
        if dry_run:
            ...
            for signature, events_group in list(groups.items())[:3]:
                ...
                print(f"\n📌 {signature}")
                print(f"   ✅ KEEP: [{keep_event.get('id')}] {keep_event.get('title', 'No title')[:50]}...")
                for event in remove_events[:3]:  # Show first 3
                    print(f"   ❌ REMOVE: [{event.get('id')}] {event.get('title', 'No title')[:50]}...")
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean up duplicates using session-based auth")
