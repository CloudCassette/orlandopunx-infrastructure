...
def monitor_duplicates():
    ...
    if duplicates:
        ...
        # Save report
        report_file = f"duplicate_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def continuous_monitor(interval_minutes=60):
    ...
    print(f"🚀 Starting continuous duplicate monitoring (every {interval_minutes} minutes)")
...
def cleanup_suggestions(duplicates):
    ...
    for signature, dup_events in duplicates.items():
        ...
        print(f"\n📌 Group: {signature}")
        print(f"   KEEP: [{keep_event.get('id')}] {keep_event.get('title', 'No title')}")
        
        for remove_event in remove_events:
            event_id = remove_event.get('id')
            print(f"   REMOVE: [{event_id}] {remove_event.get('title', 'No title')}")
            print(f"      Command: curl -X DELETE 'http://localhost:13120/api/event/{event_id}' -H 'Authorization: Bearer YOUR_TOKEN'")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interval', type=int, default=60, help='Monitor interval in minutes')
    parser.add_argument('--suggest-cleanup', action='store_true', help='Show cleanup suggestions')
