            ...
            for key, group in groups.items():
                if len(group) > 1:
                    strategy_duplicates.append(
                        {"key": key, "events": group, "count": len(group)}
                    )
        ...
        if exact_duplicates:
            ...
            for dup_group in exact_duplicates:
                ...
                for i, event in enumerate(sorted_events):
                    ...
                    if i == 0:
                        print(
                            f"   🟢 KEEP   ID:{event_id} | {place_name} | {start_time}"
                        )
                    else:
                        print(
                            f"   🔴 DELETE ID:{event_id} | {place_name} | {start_time}"
                        )
        ...
        for i, item in enumerate(deletion_list, 1):
            ...
            try:
                delete_response = self.session.delete(
                    f"{self.gancio_url}/api/event/{event_id}"
                )
