...
class VenueValidationSystem:
    ...
    def fix_event_venue(self, event_id: int, venue_info: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                print(
                    f"❌ Failed to fix venue for event {event_id}: {response.status_code}"
                )
                return False
    ...
    def run_venue_validation(self, fix_issues=False) -> Dict:
        ...
        print(
            f"   Events with null place name: {len(analysis['events_with_null_place_name'])}"
        )
        print(
            f"   Events with empty place name: {len(analysis['events_with_empty_place_name'])}"
        )
        print(
            f"   Events with invalid place ID: {len(analysis['events_with_invalid_place_id'])}"
        )
        ...
        if fix_issues and issues:
            ...
            for issue_event in issues:
                ...
                venue_info = (
                    self.default_venue
                )  # Could be more intelligent based on event content
...
def main():
    ...
    if args.analyze or args.fix or args.report:
        ...
        print(
            "Use --analyze to check venue data, --fix to repair issues, --report for detailed report"
        )
