...
def investigate_gancio_admin():
    ...
    for base_url in base_urls:
        try:
            response = session.post(f"{base_url}/login", data={'email': email, 'password': password}, allow_redirects=True)
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if isinstance(data, list) and data:
                                new_events = len([e for e in data if e.get('id') not in [existing.get('id') for existing in all_events]])
                                if new_events > 0:
                                    print(f"  ✅ {endpoint}: {len(data)} events ({new_events} new)")
                                    all_events.update(json.dumps(e, sort_keys=True) for e in data)
    ...
    for admin_url in admin_urls:
        try:
            ...
            if response.status_code == 200:
                ...
                event_containers = soup.find_all(['div', 'tr', 'li'], 
                    string=re.compile(r'(event|title|pending|draft)', re.I))
                
                print(f"  📋 {admin_url}: {response.status_code} - {len(event_containers)} potential event containers")
                ...
                for script in scripts:
                    if script.string:
                        ...
                        potential_json = re.findall(r'(\[[^\[\]]*\]|\{[^{}]*\})', script.string)
                            ...
                            try:
                                ...
                                if isinstance(data, (list, dict)):
                                    ...
                                    if isinstance(data, list):
                                        event_like = [item for item in data if isinstance(item, dict) and item.get('title')]
                                        if event_like:
    ...
    if all_events_list:
        ...
        for i, event in enumerate(all_events_list[:5]):
            print(f"  {i+1}. [{event.get('id', 'no-id')}] {event.get('title', 'No title')[:50]}...")
    ...
    if len(all_events_list) > 1:
        ...
        for event in all_events_list:
            ...
            venue = str(event.get('place', {}).get('name', '') or event.get('venue', '')).lower().strip()
