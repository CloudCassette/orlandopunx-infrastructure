...
def find_approval_queue():
        ...
        if response.status_code in [200, 201]:
            ...
            for endpoint, info in results.items():
                try:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        test_events = [e for e in data if 'TEST EVENT' in e.get('title', '')]
                        if test_events:
                            print(f"🎯 Test event found in: {endpoint}")
                            test_event = test_events[0]
                            print(f"   Status: {test_event.get('status', 'no status field')}")
                            print(f"   Approved: {test_event.get('approved', 'no approved field')}")
    ...
    if results:
        ...
        main_endpoint = [ep for ep, info in results.items() if info['count'] == max_events][0]
        
        print(f"📊 Main events endpoint: {main_endpoint} ({max_events} events)")
        
        # Check for approval workflow indicators
        approval_endpoints = [ep for ep in results.keys() if 'pending' in ep or 'draft' in ep or 'admin' in ep]
