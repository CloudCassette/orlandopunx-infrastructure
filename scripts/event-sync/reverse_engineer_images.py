        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for inp in file_inputs:
                    print(f"   - {inp.get('name', 'no name')} (accept: {inp.get('accept', 'any')})")
                ...
                for form in forms:
                    ...
                    print(f"Form: action='{action}' method='{method}' enctype='{enctype}'")
    ...
    def test_csrf_and_headers(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                csrf_inputs = soup.find_all('input', {'name': ['_token', 'csrf_token', '_csrf']})
                for inp in csrf_inputs:
                    print(f"CSRF token: {inp.get('name')} = {inp.get('value', 'no value')[:20]}...")
                
                # Look for meta tags with tokens
                meta_tokens = soup.find_all('meta', {'name': ['csrf-token', '_token']})
                for meta in meta_tokens:
                    print(f"Meta token: {meta.get('name')} = {meta.get('content', 'no content')[:20]}...")
                
                # Check response headers
                print("Response headers:")
                for key, value in resp.headers.items():
                    if any(keyword in key.lower() for keyword in ['csrf', 'token', 'xsrf']):
                        print(f"   {key}: {value}")
