...
class ComprehensiveMonitor:
    ...
    def get_events_from_gancio(self) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.add_alert("ERROR", f"Failed to fetch events: {response.status_code}")
                return []
    ...
    def detect_duplicates(self, events: List[Dict]) -> Dict:
        ...
        for signature, event_list in signatures.items():
            if len(event_list) > 1:
                ...
                self.add_alert("WARNING", f"Found {len(event_list)} exact duplicates: {signature}")
        ...
        for i, event1 in enumerate(events):
            for j, event2 in enumerate(events[i+1:], i+1):
                ...
                if (event1.get('start_datetime') == event2.get('start_datetime') and
                    ...
                    if self.titles_are_similar(event1.get('title', ''), event2.get('title', '')):
                        duplicates['fuzzy_duplicates'].append([event1, event2])
                        self.add_alert("WARNING", f"Found fuzzy duplicates: {event1.get('title')[:30]}... vs {event2.get('title')[:30]}...")
        ...
        for title, event_list in title_groups.items():
            if len(event_list) > 1:
                venues = set(event.get('place', {}).get('name', '') for event in event_list)
                if len(venues) > 1:  # Same title, different venues
                    ...
                    self.add_alert("INFO", f"Same title across venues: {title[:30]}... ({len(venues)} venues)")
    ...
    def validate_venue_data(self, events: List[Dict]) -> Dict:
        ...
        for event in events:
            ...
            if place is None:
                ...
                self.add_alert("ERROR", f"Event {event_id} has null place: {title[:40]}...", event_id)
                continue
            ...
            if place_name is None:
                ...
                self.add_alert("ERROR", f"Event {event_id} has null place name: {title[:40]}...", event_id)
                continue
            
            # Check for empty place name
            if place_name == "":
                validation_results['events_with_empty_place_name'].append(event)
                self.add_alert("ERROR", f"Event {event_id} has empty place name: {title[:40]}...", event_id)
                continue
            
            # Check for invalid place ID
            if place_id is None or place_id == 0:
                validation_results['events_with_invalid_place_id'].append(event)
                self.add_alert("WARNING", f"Event {event_id} has invalid place ID: {title[:40]}...", event_id)
    ...
    def check_data_integrity(self, events: List[Dict]) -> Dict:
        ...
        for event in events:
            ...
            if start_datetime is None:
                ...
                self.add_alert("ERROR", f"Event {event_id} missing datetime: {title[:40] if title else 'No title'}...", event_id)
                continue
            ...
            if not isinstance(start_datetime, (int, float)) or start_datetime <= 0:
                ...
                self.add_alert("ERROR", f"Event {event_id} has invalid datetime: {start_datetime}", event_id)
                continue
            ...
            if start_datetime < one_year_ago:
                ...
                self.add_alert("INFO", f"Event {event_id} is old: {datetime.fromtimestamp(start_datetime).strftime('%Y-%m-%d')}")
            
            # Check for events too far in the future
            if start_datetime > one_year_future:
                integrity_results['events_far_future'].append(event)
                self.add_alert("WARNING", f"Event {event_id} is far in future: {datetime.fromtimestamp(start_datetime).strftime('%Y-%m-%d')}")
        ...
        if total_checks > 0:
            integrity_results['data_quality_score'] = max(0, int((total_checks - failed_checks) / total_checks * 100))
        else:
    ...
    def run_comprehensive_check(self) -> Dict:
        ...
        print(f"🚀 Starting comprehensive monitoring at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def create_report(self, duplicate_results: Dict, venue_results: Dict, integrity_results: Dict) -> Dict:
        ...
        report = {
            ...
            'duplicates': {
                'exact_duplicate_groups': len(duplicate_results.get('exact_duplicates', {})),
                'fuzzy_duplicate_pairs': len(duplicate_results.get('fuzzy_duplicates', [])),
                'suspicious_patterns': len(duplicate_results.get('suspicious_patterns', []))
            },
            'venue_validation': {
                'null_place_events': len(venue_results.get('events_with_null_place', [])),
                'null_place_name_events': len(venue_results.get('events_with_null_place_name', [])),
                'empty_place_name_events': len(venue_results.get('events_with_empty_place_name', [])),
                'invalid_place_id_events': len(venue_results.get('events_with_invalid_place_id', [])),
                'venue_count': len(venue_results.get('venue_distribution', {}))
            },
            'data_integrity': {
                'missing_title_events': len(integrity_results.get('events_missing_title', [])),
                'missing_datetime_events': len(integrity_results.get('events_missing_datetime', [])),
                'invalid_datetime_events': len(integrity_results.get('events_with_invalid_datetime', [])),
    ...
    def print_summary(self, report: Dict):
        ...
        print(f"\n🏢 Venue Validation:")
        print(f"   Venue data issues: {venue['null_place_events'] + venue['null_place_name_events'] + venue['empty_place_name_events']}")
        print(f"   Total venues: {venue['venue_count']}")
        
        print(f"\n📋 Data Integrity:")
        print(f"   Data issues: {integrity['missing_title_events'] + integrity['missing_datetime_events'] + integrity['invalid_datetime_events']}")
    ...
    def should_alert(self, report: Dict) -> bool:
        ...
        summary = report['summary']
        return (summary['critical_alerts'] > 0 or 
                summary['error_alerts'] > 0 or
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Comprehensive Event Monitoring System')
    parser.add_argument('--save-report', action='store_true', help='Save detailed report to file')
    parser.add_argument('--ci-mode', action='store_true', help='CI/CD mode (exit with error codes)')
