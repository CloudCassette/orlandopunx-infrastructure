...
class SyncIssuesInvestigator:
    ...
    def analyze_gancio_events(self, events):
        ...
        for event in events:
            ...
            place_name = event.get('place', {}).get('name', f'Place ID {place_id}') if isinstance(event.get('place'), dict) else f'Place ID {place_id}'
    ...
    def check_authentication(self):
        ...
        auth_methods = [
            ("POST /api/auth/login", f"{self.gancio_base_url}/api/auth/login", {'email': email, 'password': password}),
            ("POST /auth/login", f"{self.gancio_base_url}/auth/login", {'email': email, 'password': password})
        ]
    ...
    def check_gallery_flyers(self):
        ...
        flyer_files = [f for f in os.listdir(flyers_dir) if f.endswith(('.jpg', '.jpeg', '.png'))]
        ...
        if conduit_flyers:
            ...
            conduit_flyers.sort(key=lambda x: os.path.getmtime(os.path.join(flyers_dir, x)), reverse=True)
            for flyer in conduit_flyers[:5]:
                mtime = datetime.fromtimestamp(os.path.getmtime(os.path.join(flyers_dir, flyer)))
