...
class GancioDuplicateCleanup:
    ...
    def analyze_duplicates(self, events):
        ...
        for event in events:
            ...
            place_name = (
                place.get("name", "Unknown") if isinstance(place, dict) else str(place)
            )
            ...
            date_key = (
                f"{title}|{str(start_date)[:10]}" if start_date else f"{title}|no_date"
            )
            ...
            for key, group_events in groups.items():
                if len(group_events) > 1:
                    strategy_duplicates.append(
                        {"key": key, "events": group_events, "count": len(group_events)}
                    )
                ...
                for dup_group in duplicates:
                    ...
                    if strategy == "exact_title" and len(duplicates) <= 10:
                        ...
                        sorted_events = sorted(
                            group_events, key=lambda x: x.get("id", 0)
                        )
                        ...
                        for i, event in enumerate(sorted_events):
                            ...
                            print(
                                f"      {marker} ID:{event_id} | {place_name} | Created:{created}"
                            )
    ...
    def delete_event_safely(self, event_id, dry_run=True):
        ...
        try:
            ...
            if delete_response.status_code in [200, 204]:
                ...
                print(
                    f"   ❌ Failed to delete ID {event_id}: HTTP {delete_response.status_code}"
                )
                return False
...
'''
        ...
        print(
            f"✅ Created duplicate prevention script: scripts/event-sync/duplicate_prevention.py"
        )
