...
class VenueEnforcer:
    ...
    def ensure_venue_assignment(self, event_data: Dict) -> Dict:
        ...
        if not event_data.get('place_id') or event_data.get('place_id') == 0:
            print(f"🚨 CRITICAL: Event still missing venue after assignment: {event_data.get('title', 'Unknown')}")
...
class RobustDeduplicator:
    ...
    def create_content_hash(self, event: Dict) -> str:
        ...
        content = {
            'title': self.normalize_title(event.get('title', '')),
            'venue': (event.get('venue') or event.get('place', {}).get('name', '')).strip().lower(),
            'start_time': event.get('start_datetime', 0),
    ...
    def create_event(self, event_data: Dict) -> bool:
        ...
        try:
            ...
            if not event_data.get('place_id'):
                raise ValueError(f"🚨 CRITICAL: Event missing venue assignment after enforcement: {event_data.get('title')}")
            ...
            if response.status_code in [200, 201]:
                ...
                print(f"✅ Created event at {venue_name}: {event_data.get('title', 'Unknown')[:50]}...")
                return True
            else:
                print(f"❌ Failed to create event: {response.status_code} - {response.text[:100]}")
                return False
    ...
    try:
        ...
        for event in events:
            if not event.get('venue'):
                ...
                print(f"🎯 Assigned Conduit venue to: {event.get('title', 'Unknown')[:40]}...")
...
def main():
    ...
    for event in all_new_events:
        ...
        if is_dup:
            ...
            print(f"⏭️ Skipping duplicate ({match_type}): {event.get('title', 'Unknown')[:40]}...")
        else:
