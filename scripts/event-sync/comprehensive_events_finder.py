...
def find_all_events():
    ...
    try:
        ...
        for badge in badge_elements:
            if "events" in badge.text.lower() and "849" in badge.text:
                ...
                try:
                    ...
                    clickable_events = driver.find_elements(
                        By.XPATH,
                        "//*[contains(text(), 'EVENTS') or contains(text(), 'Events')]",
                    )
        ...
        data_tables = driver.find_elements(
            By.CSS_SELECTOR, ".v-data-table, [class*='data-table']"
        )
        ...
        for i, table in enumerate(data_tables):
            ...
            for j, row in enumerate(rows[:10]):  # Check first 10 rows
                ...
                if (
                    ...
                    and len(text) > 10
                    and not any(
                        skip in text.lower() for skip in ["select", "actions", "header"]
                    )
                ):
        ...
        for i, card in enumerate(cards):
            ...
            if text and len(text) > 20 and len(text) < 1000:  # Reasonable content size
                all_potential_events.append(
                    {"text": text, "element": card, "source": f"card-{i+1}"}
                )
        ...
        for i, div in enumerate(content_divs[:20]):  # Check first 20
            text = div.text.strip()
            all_potential_events.append(
                {"text": text, "element": div, "source": f"content-div-{i+1}"}
            )
...
if __name__ == "__main__":
    ...
    if total_found > 0 and total_found < 849:
        print(
            "💡 There are likely more events that require pagination or different navigation"
        )
    elif total_found == 0:
        print(
            "💡 Events may be loaded dynamically or require specific user interactions"
        )
