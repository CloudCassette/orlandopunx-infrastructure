...
class GancioSyncWithImages:
    ...
    def authenticate(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.gancio_base_url}/login", data=login_data
            )
    ...
    def create_event_with_image(self, event_data):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.gancio_base_url}/add", data=form_data, files=files, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                if (
                    "Nuovo evento" not in response.text
                ):  # If not showing the "new event" form again
                    print(f"   ✅ {event_data['title']}")
                    return True
                else:
                    print(
                        f"   ⚠️  Form submission returned form page (check validation)"
                    )
                    return False
            else:
