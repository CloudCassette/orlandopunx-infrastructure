...
class VenueAssignmentFixer:
    def __init__(self, gancio_base_url="http://localhost:13120"):
        ...
        self.venue_mappings = {
            ...
            "conduit": {"id": 5, "name": "Conduit", "address": "22 S Magnolia Ave, Orlando, FL 32801"},
            ...
            "22 s magnolia": {"id": 5, "name": "Conduit", "address": "22 S Magnolia Ave, Orlando, FL 32801"},
            "1042 n mills": {"id": 1, "name": "Will's Pub", "address": "1042 N. Mills Ave. Orlando, FL 32803"},
            "1842 winter park": {"id": 4, "name": "Stardust Video & Coffee", "address": "1842 Winter Park Rd"},
            
            # Alternative names/variations
            "the conduit": {"id": 5, "name": "Conduit", "address": "22 S Magnolia Ave, Orlando, FL 32801"},
            "conduit bar": {"id": 5, "name": "Conduit", "address": "22 S Magnolia Ave, Orlando, FL 32801"},
        }
    ...
    def get_venue_info(self, venue_name: str, event: Dict = None) -> Dict:
        ...
        if event:
            ...
            if detected_venue:
                print(f"🎯 Intelligently detected venue: {detected_venue['name']} for event: {event.get('title', 'Unknown')[:40]}...")
    ...
    def fix_event_venue(self, event_id: int, venue_info: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                print(f"❌ Failed to fix venue for event {event_id}: {response.status_code} - {response.text[:100]}")
                return False
    ...
    def run_automated_fixes(self, analysis: Dict, dry_run=True) -> Dict:
        ...
        print(f"🔧 {'DRY RUN - Would fix' if dry_run else 'Fixing'} {len(problematic_events)} venue issues...")
        ...
        for problem in problematic_events:
            ...
            print(f"{'[DRY RUN] Would fix' if dry_run else 'Fixing'} event {event_id}: {title[:40]}... → {suggested_fix['name']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Venue Assignment Verification and Fixer')
    parser.add_argument('--analyze', action='store_true', help='Analyze venue assignments')
    parser.add_argument('--fix', action='store_true', help='Fix venue issues automatically')
    parser.add_argument('--dry-run', action='store_true', default=True, help='Dry run mode (default)')
    parser.add_argument('--force', action='store_true', help='Actually perform fixes')
    parser.add_argument('--generate-commands', action='store_true', help='Generate manual fix commands')
    parser.add_argument('--create-template', action='store_true', help='Create venue enforcement template')
