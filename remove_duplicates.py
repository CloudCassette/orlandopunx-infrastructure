    ...
    try:
        ...
        events_element = wait.until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    "//div[contains(@class, 'v-badge') and .//span[text()='849']]/..",
                )
            )
        )
...
def remove_duplicate_events(driver, duplicates, max_removals=5):
    ...
    for i, duplicate in enumerate(duplicates[:max_removals]):
        try:
            print(
                f"Removing duplicate {i+1}/{min(len(duplicates), max_removals)}: {duplicate['title']}"
            )
            ...
            try:
                confirm_button = WebDriverWait(driver, 5).until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            "//button[contains(., 'Yes') or contains(., 'Confirm') or contains(., 'Delete')]",
                        )
                    )
                )
