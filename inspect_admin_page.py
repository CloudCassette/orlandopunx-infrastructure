...
def inspect_admin_page():
    ...
    try:
        ...
        number_elements = driver.find_elements(
            By.XPATH,
            "//*[contains(text(), '849') or contains(text(), '800') or contains(text(), '8')]",
        )
        ...
        clickables = driver.find_elements(
            By.XPATH,
            "//*[contains(@class, 'v-btn') or contains(@class, 'clickable') or @role='button']",
        )
                ...
                try:
                    ...
                    if text:
                        ...
                        try:
                            ...
                            new_tables = driver.find_elements(
                                By.CLASS_NAME, "v-data-table"
                            )
                            if new_tables:
                                print(
                                    f"    🎯 SUCCESS! Now found {len(new_tables)} data tables after clicking!"
                                )
                                ...
                                if rows:
                                    ...
                                    for j, row in enumerate(rows[:3]):  # Show first 3
                                        ...
                                        if len(cells) >= 3:
                                            ...
                                            print(
                                                f"      Row {j}: '{title}' at '{place}' on '{when}'"
                                            )
