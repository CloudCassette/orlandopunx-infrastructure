    ...
    try:
        ...
        for i, event in enumerate(kept_originals[:10]):
            ...
            if duplicates_count > 0:
                print(
                    f"   {i+1}. ID: {event['id']}, Slug: {event['slug']} (+ {duplicates_count} duplicates will be removed)"
                )
        ...
        if DRY_RUN:
            ...
            print(
                f"   To perform actual deletions, we need to use the browser interface."
            )
        ...
        if not DRY_RUN:
            ...
            for i, event in enumerate(target_deletions):
                print(
                    f"[{i+1}/{len(target_deletions)}] Deleting event {event['id']} ({event['slug'][:50]}...)"
                )
            ...
            if deleted_count > 0:
                ...
                print(
                    f"   Your calendar should now show approximately {remaining_total} events."
                )

                if len(duplicates_to_delete) > MAX_DELETIONS:
                    remaining_dups = len(duplicates_to_delete) - deleted_count
                    print(
                        f"   Note: {remaining_dups} duplicates remain. Run script again to continue cleanup."
                    )
