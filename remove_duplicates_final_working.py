...
def setup_driver(headless=False):
    """Setup Chromium WebDriver with explicit binary path"""
    options = Options()
    ...
    try:
        # Use ChromeDriver service explicitly
...
def login_to_gancio(driver, base_url, email, password, timeout=30):
    ...
    try:
        ...
        # Find and click login button (type=submit)
        ...
        for by, selector in admin_indicators:
            try:
                ...
                if elements:
                    print(
                        f"✓ Found admin indicator: {selector} ({len(elements)} elements)"
                    )
                    admin_found = True
                    break
            except:
...
def navigate_to_events_page(driver, timeout=30):
    ...
    try:
        # Look for the Events tab with badge
        selectors_to_try = [
        ...
        for selector in selectors_to_try:
            try:
                events_element = wait.until(
                    EC.element_to_be_clickable((By.XPATH, selector))
                )
                events_element.click()
                time.sleep(10)  # Wait longer for SPA to load events table
                clicked = True
                print(f"✓ Clicked events element")
                break
            except:
                continue
                ...
                if event_key in events_seen:
                    ...
                    try:
                        remove_button = row.find_element(
                            By.XPATH,
                            ".//button[contains(@class, 'error--text') and .//span[text()='Remove']]",
                        )
                        duplicates.append(
                            {
                                "row_index": i,
                        ...
                        print(
                            f"  → Duplicate found: '{title}' at '{place}' on '{when}'"
                        )
    ...
    for i, duplicate in enumerate(duplicates[:max_removals]):
        try:
            ...
            driver.execute_script(
                "arguments[0].scrollIntoView(true);", duplicate["remove_button"]
            )
            time.sleep(1)
            ...
            try:
                ...
                if not confirmed:
                    print(
                        "  ! No confirmation dialog found - removal may have been immediate"
                    )
...
def main():
    print("🚀 Gancio Duplicate Event Remover - Final Fixed Version")
    print("=" * 55)
