...
class GancioBrowserAPI:
    ...
    def login(self):
        ...
        try:
            ...
            login_button = self.driver.find_element(
                By.CSS_SELECTOR, "button[type='submit'], .v-btn, button"
            )
            login_button.click()
            ...
            if any(
                ...
                for indicator in ["events", "admin", "dashboard", "manage"]
            ):
    ...
    def click_events_tab(self):
        ...
        try:
            ...
            if any(
                word in page_source
                for word in ["unconfirmed", "pending", "event", "title", "date"]
            ):
    ...
    def delete_event_by_id(self, event_id):
        ...
        try:
            ...
            try:
                ...
                if event_elements:
                    ...
                    for element in event_elements:
                        ...
                        delete_candidates = parent.find_elements(
                            By.XPATH,
                            ".//button[contains(@class, 'delete') or contains(., 'Delete') or contains(., 'Remove') or contains(@class, 'trash') or contains(@class, 'remove')]",
                        )
            ...
            if not delete_button:
                try:
                    delete_candidates = self.driver.find_elements(
                        By.XPATH,
                ...
                try:
                    ...
                    for row in rows:
                        if event_id in row.text:
                            delete_candidates = row.find_elements(
                                By.XPATH,
                                ".//button[contains(@class, 'delete') or contains(., 'Delete') or contains(., 'Remove')]",
                            )
                            if delete_candidates:
                                delete_button = delete_candidates[0]
                                event_found = True
            ...
            try:
                ...
                confirm_button = WebDriverWait(self.driver, 3).until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            "//button[contains(., 'Confirm') or contains(., 'Yes') or contains(., 'Delete') or contains(., 'OK') or contains(@class, 'confirm')]",
                        )
                    )
                )
    ...
    try:
        ...
        if deleted_count > 0:
            ...
            if len(duplicates_to_delete) > MAX_DELETIONS:
                ...
                print(
                    f"   💡 Run this script again to continue cleanup, or increase MAX_DELETIONS."
                )
                print(
                    f"   🕐 Estimated time for full cleanup: ~{(remaining * 10) // 60} minutes"
                )
        else:
